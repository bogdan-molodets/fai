...
export class RtmlsService {
  ...
  readLogs(lastSync: string, flightId: string): Observable<LogResponse> {
  ...
  createTarget(flightId: string, targetId: string): Observable<any> {
    return this.httpClient.post<any>(environment.apiUrl + 'target', { params: { flight_id: flightId, target_id: targetId } });
  }

  getTargetStatus(flightId: string, targetId: string): Observable<Target> {
  ...
  createTargetAzimuthPoint(flightId: string, targetId: string, markerId: string): Observable<any> {
    return this.httpClient.post<any>(environment.apiUrl + 'target/' + targetId + '/ap', { flight_id: flightId, marker_id: markerId });
  }

  updateAzimuthPointPositionState(flightId: string, targetId: string, state: string): Observable<any> {
    return this.httpClient.post<any>(environment.apiUrl + 'target/' + targetId + '/ap', { state: state }, { params: { flight_id: flightId } });
  }

  // cp
  getTargetCentralPointState(flightId: string, targetId: string): Observable<MarkerState> {
    return this.httpClient.get<MarkerState>(environment.apiUrl + 'target/' + targetId + '/cp', { params: { flight_id: flightId } }).map(res=>{
      return new MarkerStateSerialzer().deserialize(res);
    });
  }
  ...
  updateCentralPointPositionState(flightId: string, targetId: string, state: string): Observable<any> {
    return this.httpClient.post<any>(environment.apiUrl + 'target/' + targetId + '/cp', { state: state }, { params: { flight_id: flightId } });
  }
  //rs
  getReferenceStationState(flightId: string, targetId: string): Observable<MarkerState> {
    return this.httpClient.get<MarkerState>(environment.apiUrl + 'target/' + targetId + '/rs', { params: { flight_id: flightId } }).map(res=>{
      return new MarkerStateSerialzer().deserialize(res);
    });
  }
  ...
  updateReferenceStationState(flightId: string, targetId: string, state: string): Observable<any> {
    return this.httpClient.post<any>(environment.apiUrl + 'target/' + targetId + '/rs', { state: state }, { params: { flight_id: flightId } });
  }
